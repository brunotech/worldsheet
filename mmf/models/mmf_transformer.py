...
class MMFTransformerEmbeddings(nn.Module):
    ...
    def build_layers(self):
        ...
        for modality in self.model_config.modalities:
            ...
            setattr(self, f"{modality.key}_dropout", nn.Dropout(hidden_dropout_prob))
    ...
    def init_weights(self, transformer: Type[nn.Module]):
        for modality in self.model_config.modalities:
            if modality.type == "text":
                ...
                setattr(self, f"{modality.key}_layer_norm", transformer.embeddings.LayerNorm)

            pos_embedding_layer = getattr(self, f"{modality.key}_pos_embedding")
    ...
    def forward(
        ...
        list_embeddings = []
        for modality in self.model_config.modalities:
            total_embedding = getattr(self, f"{modality.key}_embedding")(
                input_ids[modality.key]
            )
            if modality.key not in position_ids:
                total_embedding += getattr(self, f"{modality.key}_pos_embedding")(
            ...
            layer_norm_layer = getattr(self, f"{modality.key}_layer_norm")
            dropout_layer = getattr(self, f"{modality.key}_dropout")
...
class MMFTransformer(BaseTransformer):
    ...
    def preprocess_sample(self, sample_list: Dict[str, Any]) -> BaseTransformerInput:
        ...
        for idx, modality in enumerate(self.config.modalities):
            if modality.type == "image":
                ...
                masks[modality.key] = (
                    sample_list.input_mask[:, idx]
                    if sample_list.input_mask.dim() > 2
                    else sample_list.input_mask
                )
    ...
    def forward(self, sample_list: Dict[str, Any]) -> Dict[str, Tensor]:
        ...
        masks = [output.masks[modality.key] for modality in self.config.modalities]
    ...
    def postprocess_output(self, output: Tensor) -> Dict[str, Tensor]:
        ...
        """
        return {"scores": output.contiguous().view(-1, self.config.num_labels)}
